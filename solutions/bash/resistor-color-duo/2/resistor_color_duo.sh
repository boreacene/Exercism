...
validate() {
  ...
  case "$caller" in
    ...
    spec:retrieve_value)
      local range_max=$((${#COLOR_VALUE[@]} - 1))
      local valid_codes="^(${COLOR_VALUE[*]})$"
      valid_codes=${valid_codes// /|}
      readonly valid_codes range_max

      [[ "$input" =~ $valid_codes ]] || return 7       # normally 4
      [[ "$input" =~ ^-?[0-9]$ ]] || return 7          # normally 5
      ((input >= 0 && input <= range_max)) || return 7 # normally 6
      ;;
